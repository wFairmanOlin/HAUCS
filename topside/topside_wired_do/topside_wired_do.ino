...
bool initialData = false; //data is requested from Topside
int long dataTimer = 0;
...
//    Serial.print("data: ");
    for (int i = 0; i < length; i++){
      ptx_buffer[ptx_idx ++] = *(pData + i);
//      Serial.print(*(pData + i), HEX);
//      Serial.print(" ");
    }
//    Serial.println();
    Serial.print("PTX IDX: "); Serial.println(ptx_idx);
    newData = true;
}
...
//    doConnect = false;
  ...
  //send data if data has been collected but there have been no updates
  if (((millis() - dataTimer) > 8000) && (ptx_idx != 0)){
    dataTimer = millis();
    Serial.println("timer triggered sendData");
    sendData = true;
  }
 ...
 */
 void sendLora(){
    ...
    int loraIdx = 0;
    if (ptx_idx > 0){
      lora_buffer[loraIdx++] = 4;
      lora_buffer[loraIdx++] = ptx_idx + 1 + 20;
      for (int i = 0; i < 4; i ++)
